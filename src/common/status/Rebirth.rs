...
unsafe extern "C" fn sub_rebirth_common_pre(fighter: &mut L2CFighterCommon) {
    ...
    MotionModule::change_motion(fighter.module_accessor, Hash40::new("down_wait_d"), 0.0, 1.0, false, 0.0, false, false);
...
unsafe extern "C" fn rebirth_motion_handler(fighter: &mut L2CFighterCommon) {
    ...
    if mot == hash40("down_wait_d") {
        ...
        if elapsed + 15 >= total {
            MotionModule::change_motion(fighter.module_accessor, Hash40::new("down_stand_d"), 0.0, 1.0, false, 0.0, false, false);
            // HitModule::set_status_all(fighter.module_accessor, HitStatus(*HIT_STATUS_OFF), 0);
        }
    }
    else {
        if appeal_motion_uniq_handler(fighter) {
            ...
            if MotionModule::is_end(fighter.module_accessor) {
                if [*FIGHTER_KIND_CHROM].contains(&kind)
                && ![hash40("wait"), hash40("wait_2"), hash40("wait_3"), hash40("wait_4")].contains(&mot) {
                    MotionModule::change_motion(fighter.module_accessor,Hash40::new("wait_4"), 0.0, 1.0, false, 0.0, false, false);
                }
                else if [*FIGHTER_KIND_SAMUSD, *FIGHTER_KIND_ELIGHT].contains(&kind)
                && [hash40("down_stand_d")].contains(&mot) {
                    MotionModule::change_motion(fighter.module_accessor, Hash40::new("wait_4"), 0.0, 1.0, false, 0.0, false, false);
                }
                else {
...
unsafe extern "C" fn appeal_motion_uniq_handler(fighter: &mut L2CFighterCommon) -> bool {
    let mot = MotionModule::motion_kind(fighter.module_accessor);
    let check_basic_taunts = [hash40("appeal_hi_l"), hash40("appeal_hi_r"), hash40("appeal_s_l"), hash40("appeal_s_r"), hash40("appeal_lw_l"), hash40("appeal_lw_r")].contains(&mot);
    let kind = fighter.global_table[FIGHTER_KIND].get_i32();
