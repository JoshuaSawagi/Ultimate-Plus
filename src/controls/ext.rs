...
use crate::controls::consts::STICK_HISTORY_FRAMES;
...
#[derive(Copy, Clone)]
pub struct AnalogTracker {
    pub stick_x: [f32; STICK_HISTORY_FRAMES],
    pub stick_y: [f32; STICK_HISTORY_FRAMES],
}

impl AnalogTracker {
    pub const fn new() -> Self {
        Self {
            stick_x: [0.0; STICK_HISTORY_FRAMES],
            stick_y: [0.0; STICK_HISTORY_FRAMES],
        }
    }

    pub fn update(&mut self, x: f32, y: f32) {
        for i in (1..STICK_HISTORY_FRAMES).rev() {
            self.stick_x[i] = self.stick_x[i - 1];
            self.stick_y[i] = self.stick_y[i - 1];
        }
        self.stick_x[0] = x;
        self.stick_y[0] = y;
    }

    pub fn was_flicked_left(&self, threshold: f32) -> bool {
        self.stick_x[1] > threshold && self.stick_x[0] < -threshold
    }

    pub fn was_flicked_right(&self, threshold: f32) -> bool {
        self.stick_x[1] < -threshold && self.stick_x[0] > threshold
    }

    pub fn neutral_to_dir(&self, dir: f32, threshold: f32) -> bool {
        self.stick_x[2].abs() < 0.1 && (self.stick_x[0] * dir) > threshold
    }

    pub fn is_dash_dance(&self, threshold: f32) -> bool {
        (self.stick_x[3] > threshold && self.stick_x[0] < -threshold)
            || (self.stick_x[3] < -threshold && self.stick_x[0] > threshold)
    }

    pub fn is_dash_pivot(&self, dir: f32) -> bool {
        self.stick_x[1] * dir > 0.5 && self.stick_x[0] * dir < -0.5
    }

    pub fn is_walk_pivot(&self, dir: f32) -> bool {
        self.stick_x[2].abs() < 0.2 && self.stick_x[0] * dir < -0.5
    }
}
...
impl StickHistory {
    ...
    pub fn get(&self, frames_ago: usize) -> Option<(f32, f32)> {
        if frames_ago < STICK_HISTORY_FRAMES {
            Some((self.stick_x[frames_ago], self.stick_y[frames_ago]))
        } else {
            None
        }
    }
    ...
    pub fn clear(&mut self) {
        self.stick_x = [0.0; STICK_HISTORY_FRAMES];
        self.stick_y = [0.0; STICK_HISTORY_FRAMES];
    }
}
