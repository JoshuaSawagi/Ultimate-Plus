...
unsafe extern "C" fn special_n_charge_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    if !MotionModule::is_end(fighter.module_accessor) {
        if ControlModule::check_button_on(fighter.module_accessor, *CONTROL_PAD_BUTTON_ATTACK) {
            VarModule::on_flag(fighter.object(), vars::ridley::instance::SPECIAL_N_ATTACK);
...
unsafe extern "C" fn special_n_shoot_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    if VarModule::is_flag(fighter.object(), vars::ridley::instance::SPECIAL_N_ATTACK) {
        VarModule::off_flag(fighter.object(), vars::ridley::instance::SPECIAL_N_ATTACK);
        WorkModule::set_int64(fighter.module_accessor, hash40("special_n_attack") as i64, *FIGHTER_STATUS_WORK_ID_UTILITY_WORK_INT_MOT_KIND);
        WorkModule::set_int64(fighter.module_accessor, hash40("special_air_n_attack") as i64, *FIGHTER_STATUS_WORK_ID_UTILITY_WORK_INT_MOT_AIR_KIND);
