...
pub fn install(agent: &mut Agent) {
    agent.status(Main, *FIGHTER_STATUS_KIND_GLIDE_START, metaknight_glide_start_main);
    agent.status(Main, *FIGHTER_STATUS_KIND_GLIDE_END, metaknight_glide_end_main);
    agent.status(Main, *FIGHTER_STATUS_KIND_GLIDE_ATTACK, metaknight_glide_attack_main);
}