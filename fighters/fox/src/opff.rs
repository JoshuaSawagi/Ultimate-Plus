...
unsafe fn shine_jump_cancel(fighter: &mut L2CFighterCommon, frame: f32) {
    if fighter.is_status(*FIGHTER_STATUS_KIND_SPECIAL_LW) && frame < 4.0 && WorkModule::get_int(fighter.module_accessor, *FIGHTER_INSTANCE_WORK_ID_INT_FRAME_IN_AIR) <= 1 {
...
pub unsafe fn moveset(fighter: &mut L2CFighterCommon, boma: &mut BattleObjectModuleAccessor, id: usize, cat: [i32 ; 4], status_kind: i32, situation_kind: i32, motion_kind: u64, stick_x: f32, stick_y: f32, facing: f32, frame: f32) {
    ...
    shine_jump_cancel(fighter, frame);
